; G6500.g: PROBE WORK PIECE - BOSS
;
; Meta macro to gather operator input before executing a
; boss probe cycle (G6500.1). The macro will explain to
...
; and hit OK, at which point the boss probe cycle will
; be executed.

; Display description of boss probe if not already displayed this session
if { !global.mosExpertMode && !global.mosDescBossDisplayed }
    ...
    M291 P"You will be asked to enter an approximate <b>boss diameter</b> and <b>clearance distance</b>.<br/>These define how far the probe will move towards the centerpoint, without being triggered, before erroring." R"MillenniumOS: Probe Boss" T0 S2
    M291 P"You will then jog the touch probe over the approximate center of the boss.<br/><b>CAUTION</b>: Jogging in RRF does not watch the probe status, so you could damage the probe if moving in the wrong direction!" R"MillenniumOS: Probe Boss" T0 S2
    ...
    if { result != 0 }
        ...
        M99
    set global.mosDescBossDisplayed = true

...
if { exists(param.X) && (param.X < move.axes[0].min || param.X > move.axes[0].max) }
    abort { "Target probe position is outside machine limit on X axis. Reduce overtravel if probing away, or clearance if probing towards, the center of the table" }
if { exists(param.Y) && (param.Y < move.axes[1].min || param.Y > move.axes[1].max) }
    abort { "Target probe position is outside machine limit on Y axis. Reduce overtravel if probing away, or clearance if probing towards, the center of the table" }
if { exists(param.Z) && (param.Z < move.axes[2].min || param.Z > move.axes[2].max) }
    abort { "Target probe position is outside machine limit on Z axis." }